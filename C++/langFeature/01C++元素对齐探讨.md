...
预设对齐大小，就是#pragram pack()指令的预设值，一般为1，2，4，8，16。本次讨论一般设为**z**。
...
- 调用`#pragma()`。eg:`#pragma(4); //设置预设的对齐大小为4`
...
**结论1**：
假设，无`#pragma()`情况下某结构体的对齐大小为x1;
设，存在`#pragma(z)`指令，某结构体的对齐大小为x; 则 x = min(x1, z);
